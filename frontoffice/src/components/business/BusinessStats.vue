<template>
  <section class="stats-section" id="stats-section">
    ...
    <p class="stats-subtitle">
      A confiança de centenas de parceiros, refletida em cada entrega.
    </p>
    ...
    <div v-else-if="error" class="stats-grid">
      ...
      <div class="stat-card stat-card--muted">
        ...
        <div class="stat-content">
          <div class="stat-value">—</div>
          <div class="stat-label">Satisfação Global</div>
        </div>
      </div>
...
async function fetchStats() {
  ...
  try {
    // Fetch all orders with populated relations
    const response = await strapiGet("/api/orders?populate=*&pagination[pageSize]=100");
    const orders = response?.data ?? [];

    // 1. Total completed deliveries
    ...
    console.warn("[BusinessStats] Could not fetch stats:", err.message);
    error.value = true;
    loading.value = false;
  }
}
