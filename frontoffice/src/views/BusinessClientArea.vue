<template>
  <div class="client-area-container">
    <!-- Header Navigation (Business context) -->
    <BusinessHeader />
    ...
    <main class="client-area-main max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-10">
      ...
      <section class="profile-header">
        ...
        <div class="header-text">
          <h1 class="profile-name">{{ companyName }}</h1>
          <p class="profile-role">Conta Corporativa Party2Go</p>
        </div>
      </section>
      ...
      <div class="row m-0 w-100 g-4 mt-4 align-items-start">
        ...
        <div class="col-12 col-lg-8 left-column flex flex-col gap-8">
          ...
          <section class="active-order-section" v-if="activeOrder">
            <div class="order-header">
              <div class="order-info">
                <div class="order-badge">ENTREGA ATUAL</div>
                <h1 class="order-title">Entrega #{{ activeOrder.order_id }}</h1>
              </div>
            </div>
            ...
            <!-- Track Button -->
            <button class="btn-track-order" @click="$router.push(`/business/track-order/${activeOrder.documentId}`)">
              SEGUIR EM TEMPO REAL
            </button>
          </section>

          <!-- Empty State for Active Delivery -->
          <section class="active-order-section" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px 24px;" v-else>
            <i class="bi bi-box-seam" style="font-size: 40px; color: var(--border-light); margin-bottom: 8px;"></i>
            <p style="font-family: 'Public Sans', sans-serif; font-weight: 600; color: var(--light-text); margin: 0;">Não tem nenhuma entrega ativa no momento.</p>
          </section>

          <!-- Past Deliveries Section -->
          <section class="past-deliveries-section">
            <div class="section-header">
              <h2 class="section-title">Entregas Passadas</h2>
            ...
            <div class="orders-list" v-if="pastOrders.length > 0">
              <div class="order-card" v-for="order in displayedPastOrders" :key="order.id">
                <div class="order-card-header">
                  ...
                  <p class="order-price">{{ order.price ? order.price + '€' : (order.numProducts + ' itens') }}</p>
                </div>
            ...
            <!-- Empty State for Past Deliveries -->
            <div v-else style="padding: 32px 0; text-align: center;">
              <p style="font-family: 'Public Sans', sans-serif; color: var(--light-text); margin: 0;">Ainda não realizou nenhuma entrega concluída.</p>
            </div>
          </section>
        ...
        <!-- Right Column (Company Data - 1/3 width) -->
            ...
            <div class="info-list">
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <span v-if="!isEditingProfile" class="info-value">{{ companyName || '—' }}</span>
                  <input v-else v-model="companyName" type="text" class="info-input" placeholder="Nome da empresa" />
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <span v-if="!isEditingProfile" class="info-value">{{ firstName || '—' }}</span>
                  <input v-else v-model="firstName" type="text" class="info-input" placeholder="Primeiro nome" />
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <span v-if="!isEditingProfile" class="info-value">{{ lastName || '—' }}</span>
                  <input v-else v-model="lastName" type="text" class="info-input" placeholder="Apelido" />
                </div>
              </div>

              <!-- NIF -->
              <div class="info-row">
                <div class="info-icon-wrap"><i class="bi bi-card-text info-icon"></i></div>
                <div class="info-content">
                  <span class="info-label">NIF</span>
                  <span v-if="!isEditingProfile" class="info-value">{{ nif || '—' }}</span>
                  <input v-else v-model="nif" type="text" class="info-input" placeholder="NIF" />
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <span v-if="!isEditingProfile" class="info-value">{{ email || '—' }}</span>
                  <input v-else v-model="email" type="email" class="info-input" placeholder="Email corporativo" />
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <span v-if="!isEditingProfile" class="info-value">{{ phone || '—' }}</span>
                  <input v-else v-model="phone" type="tel" class="info-input" placeholder="Telefone" />
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  <span class="info-label">Morada Sede</span>
                  <span v-if="!isEditingProfile" class="info-value">{{ addressStreet || '—' }}</span>
                  <input v-else v-model="addressStreet" type="text" class="info-input" placeholder="Nome da rua" />
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <span v-if="!isEditingProfile" class="info-value">{{ addressDetails || '—' }}</span>
                  <input v-else v-model="addressDetails" type="text" class="info-input" placeholder="Nº, andar" />
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <span v-if="!isEditingProfile" class="info-value">{{ postalCode || '—' }}</span>
                  <input v-else v-model="postalCode" type="text" class="info-input" placeholder="0000-000" />
                </div>
              </div>
            </div>
          </section>
    ...
    <!-- Bottom Navigation (Business Mobile) -->
    <BusinessBottomNav />
    ...
    <div v-if="ratingOrder" class="modal-overlay" @click.self="closeRatingModal">
      <div class="modal-content">
        <div class="modal-header">
          <h3>Classificar Entrega #{{ ratingOrder.order_id }}</h3>
          <button class="btn-close" @click="closeRatingModal"><i class="bi bi-x-lg"></i></button>
        </div>
        
        <div class="modal-body">
          <p class="rating-desc">Como avalia a experiência do estafeta?</p>
          ...
          <div class="form-group mt-4">
            ...
            <textarea 
              ...
              class="form-textarea" 
              placeholder="Algum feedback detalhado?"
              rows="3"
            ></textarea>
          </div>
...
// Reactive references for Profile
...
const activeOrder = computed(() => {
  // Return the first order that is NOT completed
  return allOrders.value.find(order => {
    const s = (order.status || '').trim().toLowerCase();
    return s !== 'completed' && s !== 'concluída' && s !== 'concluido' && s !== 'concluida' && s !== 'delivered' && s !== 'entregue' && s !== 'cancelled' && s !== 'cancelado';
  });
});
...
const progressPercentage = computed(() => {
  if (progressStep.value === 1) return 0;
  if (progressStep.value === 2) return 33;
  if (progressStep.value === 3) return 66;
  if (progressStep.value === 4) return 100;
  return 0;
});
...
onMounted(async () => {
  ...
  // We check if the user is authenticated and their Firebase role is 'business'
  // (or if you updated the role to 'enterprise', we check both just in case).
  if (!user || (role !== 'business' && role !== 'enterprise')) {
  ...
  try {
    // Fetch business client data
    const response = await strapiGet(`/api/clients?filters[email][$eq]=${encodeURIComponent(userEmail)}&filters[type][$eq]=enterprise`);
    ...
    if (c) {
      ...
      nif.value = c.nif ? String(c.nif) : '';
      email.value = c.email ?? userEmail;
      phone.value = String(c.phone_number ?? '');
      ...
      try {
        ...
        console.error('[BusinessClientArea] Error fetching orders:', err);
      }
    } else {
      companyName.value = userEmail;
    }
  } catch (err) {
    console.error('[BusinessClientArea] Error fetching business data:', err);
  }
});
...
async function saveProfile() {
  if (!currentClientId.value) {
    console.error('[BusinessClientArea] Cannot save profile: No currentClientId found.');
  ...
  try {
    const payload = {
      ...
      email: email.value,
      phone_number: phone.value ? Number(phone.value) : null,
      ...
      nif: nif.value ? Number(nif.value) : null,
    };
    ...
    console.error('[BusinessClientArea] Error saving company data:', error);
    alert('Erro ao guardar os dados da empresa.');
  }
}

function cancelEdit() {
  // Revert to snapshot
  companyName.value = profileSnapshot.companyName;
  firstName.value = profileSnapshot.firstName;
  lastName.value = profileSnapshot.lastName;
  nif.value = profileSnapshot.nif;
  email.value = profileSnapshot.email;
  phone.value = profileSnapshot.phone;
  addressStreet.value = profileSnapshot.addressStreet;
  addressDetails.value = profileSnapshot.addressDetails;
  postalCode.value = profileSnapshot.postalCode;
  ...
  try {
    ...
    if (ratingForm.value.file) {
      ...
      if (uploadRes.ok) {
        ...
        console.error('[BusinessClientArea] Failed to upload image:', uploadRes.status, errText);
    ...
    console.error('[BusinessClientArea] Error submitting rating:', error);
...
.order-card:hover {
  border-color: rgba(253, 94, 58, 0.2);
