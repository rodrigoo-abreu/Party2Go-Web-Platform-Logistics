        ...
        <aside class="chat-sidebar" :class="{ 'd-none-mobile': selectedConversation }">
          ...
          <template v-else>
            ...
            <div class="sidebar-section" v-if="pastOrderConversations.length > 0">
              ...
              >
                <div class="item-avatar client">
                  <i class="bi bi-person-fill" style="opacity: 0.6;"></i>
                  <span v-if="conv.hasUnread" class="unread-badge"></span>
                </div>
                <div class="item-content">
          ...
          <template v-if="selectedConversation">
            ...
            <div class="chat-header">
              ...
              <div :class="['header-avatar', selectedConversation.type]">
                <i :class="selectedConversation.type === 'admin' ? 'bi bi-shield-lock-fill' : 'bi bi-person-fill'"></i>
              </div>
              <div class="header-text">
                <h2 class="header-name">{{ selectedConversation.name }}</h2>
                <span class="header-status">{{ selectedConversation.type === 'admin' ? 'Suporte' : 'Cliente' }}</span>
              </div>
            </div>
            ...
            <div class="messages-area" ref="messagesContainer">
              ...
              <template v-else>
                <div v-for="msg in messages" :key="msg.id" :class="['message-row', msg.sender === 'courier' ? 'row--outgoing' : 'row--incoming']">
...
async function fetchActiveOrders() {
  ...
  try {
    // 1. Fetch Orders
    const ordersPath = `/api/orders?filters[courier_id][documentId][$eq]=${courierProfile.value.documentId}&populate=*`
    ...
    // 2. Fetch all recent messages for this courier to build previews
    const messagesPath = `/api/messages?filters[courier][documentId][$eq]=${courierProfile.value.documentId}&sort=createdAt:desc&pagination[limit]=100&populate=*`
    ...
    activeOrderConversations.value = activeOrders.map(order => ({
      ...
      hasUnread: getHasUnread(order.documentId, 'client')
    }))
    ...
    console.error('[PWA Chat] fetchActiveOrders error:', err)
  } finally {
...
async function fetchMessages() {
  if (!courierProfile.value?.documentId || !selectedConversation.value) return
  
  try {
    let filters = `filters[courier][documentId][$eq]=${courierProfile.value.documentId}`
    ...
    const expectedSender = selectedConversation.value.type === 'admin' ? 'admin' : 'client'
    ...
    console.error('[PWA Chat] fetchMessages error:', err)
  } finally {
...
async function sendMessage() {
  if (!newMessage.value.trim() || sending.value || !courierProfile.value?.documentId) return
  ...
  try {
    const payload = {
      content: text,
      sender: 'courier',
      courier: courierProfile.value.documentId,
      is_read: false
    }
    if (conv.type === 'client') {
      payload.order = conv.orderDocumentId
      payload.client = conv.clientDocumentId
    }
    
    await strapiPost('/api/messages', payload)
    await fetchMessages()
  } catch (err) {
    console.error('[PWA Chat] sendMessage error:', err)
...
.chat-page {
  width: 100%;
  height: calc(100vh - 80px); /* Bottom nav height */
